---
title: "Crea, Innova y Presenta"
subtitle: "Data Storytelling + Inteligencia Artificial"
author: 
  - "Francisco Alfaro, Valeska Canales"
institute: "Dirección de Transformación Digital · UTFSM"
date: today
date-format: "MMMM YYYY"

format:
  revealjs:
    theme: [default, custom.scss]
    slide-number: true
    progress: true
    history: false
    transition: fade
    transition-speed: fast
    incremental: false
    fig-align: center
    highlight-style: github
    code-block-bg: true
    center: false
    width: 1600
    height: 800
    logo: "images/usm.png"
    scrollable: false
execute:
  echo: false
  warning: false
---

##  {.title-slide background-color="#0F2044"}

::: title-block
**Crea, Innova y Presenta**

Data Storytelling + Inteligencia Artificial
:::

::: subtitle-block
Storytelling · Visualización · IA · Narrativa con Datos\
Conferencia — 2024
:::

::: author-block
Francisco Alfaro, Valeska Canales\
Universidad Técnica Federico Santa María\
Dirección de Transformación Digital · 2026
:::

------------------------------------------------------------------------

## Agenda

::::: columns
::: {.column .incremental width="30%"}
-   

-   No

-   Nope

-   No chance
:::

::: {.column .fragment width="70%"}
<img src="images/neg.png" alt="Tachado" style="width: 50%; height: auto;"/>
:::
:::::

------------------------------------------------------------------------

## 

:::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
::: {.callout-note style="font-size: 1.5em;"}
## Storytelling principle #1

Nunca revelar el final antes de tiempo.\
[Siempre eleva la tensión y el drama]{.fragment}
:::
::::

<br>

------------------------------------------------------------------------

## Agenda (v2)

::: incremental
1.  Nunca reveles [el final]{style="background-color:black;"}\
2.  Los detalles son importantes, pero [no **todos** los detalles.]{style="background-color:black;"}\
3.  Explicar menos, [Mostrar más]{style="background-color:black;"}
4.  Tu primera versión será [horrible.]{style="background-color:black;"}\
:::

---

##  {background-image="images/background_slides3.png" background-opacity="0.3"}

::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
[DataStorytelling]{style="font-size: 1em; color: #4A9FD4;"}

[¿Qué es Storytelling?]{style="font-size: 2em; font-weight: bold;"}

:::

------------------------------------------------------------------------


## Qué es Storytelling?

::::: columns
::: {.column width="50%"}
![](images/cave.jpg){fig-align="center" width="90%"}
:::

::: {.column .fragment width="50%"}
![](images/fire.jpg){fig-align="center" width="100%"}
:::
:::::

. . .

::: {style="text-align: center; "}
🔥 Las **historias** son la primera tecnología humana.
:::

::: notes
Stories are how humans share knowledge.\
It’s our oldest learning tool — built for memory, connection, and meaning.\
Use it well.
:::

------------------------------------------------------------------------

## Qué es Data Storytelling?

<br>

::: r-stack
![](images/storytelling/dst2.png){.fragment fig-align="center" width="70%"}
:::

. . .

<br>

::: {style="text-align: center; "}
📊 **Data Storytelling** es una forma de comunicar con datos.
:::

------------------------------------------------------------------------

## Ohh, nuestros cerebros son hackables...

::: r-stack
![](images/storytelling/sketplanation2.png){.fragment .fade-in-then-out fig-align="center" width="70%"}

![](images/storytelling/got.jpg){.fragment fig-align="center" width="100%"}
:::




------------------------------------------------------------------------

## Narrativa

::::: columns
::: {.column width="60%"}
<br><br>

Usa **trucos** de Storytelling (narrativa) para crear presentaciones que serán **recordadas** y que causarán **impacto**
:::

::: {.column .fragment width="40%"}
![](images/emotions.png){fig-align="center"}
:::
:::::

. . .

::: {style="text-align: center;"}
🎭 Emociones inspiran acción
:::

::: notes
Great ideas need great communication.\
Storytelling is your toolkit to move people.\
Facts inform — emotions inspire action.
:::

------------------------------------------------------------------------

## Más que gráficos bonitos

::::: columns
::: {.column width="50%"}
![](images/dst_before.png){fig-align="center"} *🔢 No compartas números*
:::

::: {.column .fragment width="50%"}
![](images/dst_after.png){fig-align="center"} *🪶Comparte una historia*
:::
:::::

. . .

<br><br> [(C) Storytelling with Data, por Cole Nussbaumer Knaflic.]{style="font-size: 0.75em; color: gray"}

------------------------------------------------------------------------

## 

:::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
::: callout-note
## Storytelling principle #2

Los detalles son importantes, [pero no **todos** los detalles son importantes.]{.fragment}
:::
::::

::: notes
Details guide attention.\
Show less — but better.\
Focus only on what moves the story forward.
:::

------------------------------------------------------------------------


##  {background-image="images/background_slides3.png" background-opacity="0.3"}

::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
[Visualización]{style="font-size: 1em; color: #4A9FD4;"}

[El arte de una buena visualización]{style="font-size: 2em; font-weight: bold;"}

:::

------------------------------------------------------------------------

## Visualización ...

::::: columns
::: {.column .fragment width="60%"}
<br><br>

-   Gráficos claros y comprensibles
-   Elección correcta del gráfico
-   Patrones y anomalías visibles
-   Datos complejos → ideas simples
:::

::: {.column width="40%"}
![](images/viz/ojo.png){fig-align="center" width="80%"}
:::
:::::

. . .

> ⌛ Si no se entiende en 5-7 segundos, simplifica o cambia el título.

------------------------------------------------------------------------


## Ejemplos 

<br>

::::: columns
::: {.column width="50%" style="text-align: center;"}
![](images/viz/monster.jpg){fig-align="center" width="82%"}

<br> *❌ Chartjunk*
:::

::: {.column .fragment width="50%" style="text-align: center;"}
![](images/viz/viz_03.png){fig-align="center" width="100%"}

*✅ Good Chart*
:::
:::::

------------------------------------------------------------------------

## 

:::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
::: callout-note
## Storytelling principle #3

[Explicar Menos]{style="color: red"}, [Mostrar Más]{.fragment style="color: green"}
:::
::::

------------------------------------------------------------------------

## Mejores Gráficos para tus Datos

::: r-stack
<br>

![](images/storytelling/plot1.png){.fragment .fade-in-then-out fig-align="left"}

![](images/storytelling/plot2.png){.fragment fig-align="right"}
:::

. . .

[(C) Essential chart types for data visualization, by Atlassian.]{style="font-size: 0.75em; color: gray"}

::: notes
You don’t need fancy charts.\
Bar, scatter, box, heatmap, big text — that's 90% of data storytelling.\
Simple wins.
:::



## Herramientas

::: r-stack
![](images/toolsbi.png){.fragment .fade-in-then-out fig-align="center" width="100%"}


![](https://miro.medium.com/v2/resize:fit:1400/1*6BWaj0W5VZLXvvuygeJQhg.gif){.fragment .fade-in-then-out fig-align="center" width="60%"}

![](https://images.ctfassets.net/2vbtnveccz5s/2iorjhUm2wp4EEHv4kDKiV/4cb6e193184f1fc88547f25272e8f976/specialized1.gif){.fragment .fade-in-then-out fig-align="center" width="90%"}

:::


##  {background-image="images/background_slides3.png" background-opacity="0.3"}

::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
[Inteligencia Aritficial]{style="font-size: 1em; color: #4A9FD4;"}

[La AI en Data Storytelling]{style="font-size: 2em; font-weight: bold;"}

:::

------------------------------------------------------------------------

##  

<br><br>

:::::: columns
::: {.column width="35%"}
![](images/gh_before.png){.fragment fig-align="center"}
:::

::: {.column width="30%"}
![](images/proceso.svg){.fragment fig-align="center" width="100%"}
:::

::: {.column width="35%"}
![](images/gh_after.png){.fragment fig-align="center"}
:::
::::::

. . .

::: {style="text-align: center;"}
🥱 1° versión $<$ ... $<$ 😊 última versión
:::

::: notes
First versions are never perfect.\
Draft → feedback → improve.\
In 2026, AI compresses this cycle dramatically.\
What took days now takes hours.
:::

------------------------------------------------------------------------

##  

::::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
:::: {.callout-note style="font-size: 1.5em; max-width: 75%; margin: auto;"}
## Storytelling principle #4

Tu primera versión será [**siempre** horrible.]{.fragment}

::: {.fragment style="font-size: 0.65em; color: #666; margin-top: 0.75rem;"}
*...y hoy la IA te ayuda a llegar más rápido a la buena.*
:::
::::
:::::

::: notes
Bad first drafts are normal — always were.\
What changes in 2026 is the speed of iteration.\
The principle stays. The tools change.
:::

## AI tools

::: r-stack



![](https://media.beehiiv.com/uploads/asset/file/882af82d-1a22-4feb-9444-24f143c08f32/gamma.gif?t=1728863209){.fragment .fade-in-then-out fig-align="center" width="100%"}

![](https://storage.googleapis.com/gweb-cloudblog-publish/original_images/1_DSA_Simplifies_Workflows.gif){.fragment .fade-in-then-out fig-align="center" width="80%"}

![](https://miro.medium.com/v2/0*S4mDgR2LnUbrQ-rV.gif){.fragment fig-align="center" width="70%"}

:::

------------------------------------------------------------------------




##  {background-image="images/background_slides3.png" background-opacity="0.3"}

::: {style="display: flex; justify-content: center; align-items: center; height: 60vh; flex-direction: column; text-align: center;"}
[Experiencias Aplicadas]{style="font-size: 1em; color: #4A9FD4;"}

[¿Cómo aplicamos el DataStorytelling?]{style="font-size: 2em; font-weight: bold;"}

:::

##  Experiencias  {background-opacity="0.25" transition="fade-in slide-out"}


::: {style="text-align: center;" .fragment}
<img src="images/experiencias.svg" alt="Tachado" style="width: 1200px; height: 600px; border-radius: 10px;"/>
:::


---

##  Ejemplos Interactivos


<br>

::: r-stack
::: {.fragment .fade-in-then-out}
<iframe src="https://sethnut.com/ws-usm_uchile-2025/" width="1200" height="600" frameborder="0" scrolling="si" style="max-width:100%; border:1px solid #CCC; border-radius:10px;" allowfullscreen>

</iframe>
:::

::: fragment
<iframe src="https://sebastiandres.github.io/pyschool_2025/space_station/rooms/sala_00.html" width="1200" height="600" frameborder="0" scrolling="si" style="max-width:100%; border:1px solid #CCC; border-radius:10px;" allowfullscreen>

</iframe>
:::
:::

---

## Agradecimientos


::: r-stack
![](https://sethnut.com/resources/talks/es/2024-olimpiadas/images/img_03.jpeg){.fragment .fade-in-then-out fig-align="center" width="850px" height="600px"}

![](https://sethnut.com/resources/talks/es/2025-escuela/images/img_07.jpeg){.fragment .fade-in-then-out fig-align="center" width="850px" height="600px"}

![](https://sethnut.com/resources/talks/es/2025-escuela/images/img_02.jpeg){.fragment .fade-in-then-out fig-align="center" width="850px" height="600px"}

![](https://sethnut.com/resources/talks/es/2024-olimpiadas/images/img_01.jpeg){.fragment fig-align="center" width="850px" height="600px"}
:::


------------------------------------------------------------------------

# { background-image="images/horst_quarto_penguins_thankyou.png" data-state="no-logo"}

::: {style="display: flex; justify-content: center; align-items: flex-start; margin-top: -30vh; padding-top: 0vh; height: 60vh; flex-direction: column; text-align: center;"}
[Hora del Adiós]{style="font-size: 4em; color: #1e6490;" }


:::

------------------------------------------------------------------------

## Agenda (v2)

::: incremental
1.  Nunca reveles el [**final**]{.fragment style="color:grey;"}\
2.  Los detalles son importantes, pero [no **todos** los detalles]{.fragment style="color:grey;"}\
3.  Explica menos, [**muestra más**]{.fragment style="color:grey;"}\
4.  Tu primer borrador será [**horrible**]{.fragment style="color:grey;"}\
:::

::: notes
A mysterious agenda — like a good story.

Now you’ve seen how each point connects.

Simple rules to tell better stories with data.
:::

------------------------------------------------------------------------

## 🎉 ¡Gracias por Participar!

::::: columns
::: {.column width="50%"}
<br>

❓ ¿Preguntas?

👏 Responder [encuesta](https://docs.google.com/forms/d/e/1FAIpQLScrKncFmj2n8vLUOhEd9GrY_zfWBwGFEtLJFKj2lsBQ0ERxSg/viewform?usp=dialog)

🥳 Disfrutar del Evento!
:::

::: {.column width="50%" align="center"}
![](images/qr_data.png){width="400"}
:::
:::::

```{=html}
<style>
/* Ajusta el tamaño del título y subtítulo */
.reveal .slides h1 {
  font-size: 2em; /* Tamaño más pequeño para el título */
}

.reveal .slides h2 {
  font-size: 1.5em; /* Tamaño más pequeño para el subtítulo */
}

/* Ajusta el tamaño del texto en los párrafos */
.reveal .slides p {
  font-size: 0.8em; /* Texto más pequeño */
}

/* Ajusta el tamaño de las tablas */
.reveal .slides table {
  font-size: 0.8em; /* Tamaño de fuente más pequeño en las tablas */
  width: 90%; /* Ajusta el ancho de la tabla */
  margin: 0 auto; /* Centra la tabla */
}

/* Ajusta el tamaño de los bullets */
.reveal .slides ul {
  font-size: 1em; /* Tamaño de fuente más pequeño en los bullets */

}

.reveal .slide-logo {
   max-height: 3em !important;

}

</style>
```