...
::: {.column width="52%"}
![Fig. 13.2: La baseline reduce varianza significativamente — convergencia mucho más rápida. El baseline no introduce sesgo pero cambia la **tasa** de aprendizaje.](images/fig_132_baseline_comparison_crop.png)
:::
:::
...
**No aprender acciones** directamente, sino **una distribución** sobre ellas:

$$\pi(a \mid s, \boldsymbol{\theta}) = \mathcal{N}\!\left(\mu,\, \sigma^2\right)$$

$$\mu = \boldsymbol{\theta}_\mu^\top \mathbf{x}^\mu(s)$$
$$\sigma = \exp\!\left(\boldsymbol{\theta}_\sigma^\top \mathbf{x}^\sigma(s)\right)$$

> **¿Por qué $\exp$?** Garantiza $\sigma > 0$ sin restricciones sobre $\boldsymbol{\theta}_\sigma$.
:::
...
$\nabla\ln\pi$ en **dos componentes**:

**Media** $\boldsymbol{\theta}_\mu$:
$$\nabla\ln\pi_\mu = \frac{a - \mu}{\sigma^2}\,\mathbf{x}^\mu(s)$$

**Escala** $\boldsymbol{\theta}_\sigma$:
$$\nabla\ln\pi_\sigma = \!\left(\frac{(a-\mu)^2}{\sigma^2} - 1\right)\mathbf{x}^\sigma(s)$$
...
Partiendo de $\nabla v_\pi(s)$ por Bellman y promediando sobre $s \sim \mu(s)$:

$$\nabla J = \sum_s \mu(s)\sum_a \nabla\pi(a|s)\,q_\pi(s,a)$$

Aplicando $\nabla\pi = \pi \cdot \nabla\ln\pi$:

$$\nabla J = \mathbb{E}_\pi\!\left[\nabla\ln\pi(a|s)\,q_\pi(s,a)\right]$$
...
El teorema PG es **idéntico** al caso episódico:

$$\boxed{\nabla J(\boldsymbol{\theta}) = \mathbb{E}_\pi\!\left[G_t\,\nabla\ln\pi(a|s,\boldsymbol{\theta})\right]}$$

Solo cambia cómo se calcula $\nabla\ln\pi$.

**REINFORCE no cambia.** La política Gaussiana es un *plug-in*.
...
¿En **tu problema**: espacio continuo, óptimo estocástico, o política directa más natural que $Q$?
:::
:::
...
[💻 Google Colab — Notebook](https://colab.research.google.com/drive/1FJ6C7CyK7dLkocyJR_hrMPhMxnilhfpR?usp=sharing) · [🎧 NotebookLM — Capítulo 13](https://notebooklm.google.com/notebook/eead14c2-204e-4bb6-bc95-8d11c00df412)
:::
:::
